...
const App = () => {
  ...
  return (
    <ToDoContext.Provider value={value} className=' bg-black'>
      {isLoading ? (
      <div className="d-flex justify-content-center align-items-center bg-black" style={{height: '100vh'}}> <Loading /></div>
       
      ) : hasError ? <div className="d-flex justify-content-center align-items-center bg-black"><Error /></div> :(
        <>
          <Header />
          <ContentCard />
          <div>
            ...
            filteredResults.map(({ id, title }) => (
              <React.Fragment key={id}>
                <div className="d-flex justify-content-between w-50 m-auto card p-3 mb-3 bg-black text-white border-2 border-danger">
